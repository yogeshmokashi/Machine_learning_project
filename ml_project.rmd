...
Confusion matrix shows cross-validation results against test-validation dataset (40% of training).  We can see that CART model has accuracy of 57.42%, that means 42.58% out of sample error. 
...
From confusion matrix, we can see that Random Forest model as accuracy of 99.07% and out of sample error of 0.93%.  So Random Forest model is better one. 
...
Random Forest prediction model is best fit with accuracy of 99.07% as shown above.  Predictions for 20 test cases in TEST data is given below, with 'Class A' being correct way of weight lifting. 
```{r echo=TRUE}
FinalPrediction
```
